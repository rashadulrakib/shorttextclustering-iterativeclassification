...
def GenerateTrainTest2List(listtuple_pred_true_text, minP, fixedMaxP, minDelta):
 print("Initial---------")
 EvaluateByPurity(traintestFile)

 prevPercent=minP
 for itr in range(maxIterations):
  randPercent=random.uniform(minP,fixedMaxP)
  ...
  if absPercentDiff<minDelta:
   if randPercent >= minDelta:
    ...
    randPercent = max(randPercent-minDelta, minP)
...
GenerateTrainTest2List(listtuple_pred_true_text, minP, fixedMaxP, minDelta)

...
print(fixedMinP)
fixedMaxP = float(str(sys.argv[3]))*100
print(fixedMaxP)
maxTrainRatio =fixedMaxP

minDelta = float(str(sys.argv[4]))*100
print(minDelta)
...
def GenerateTrainTest2List(listtuple_pred_true_text, fixedMinP, fixedMaxP, minDelta):
 print("Initial---------")
 EvaluateByPurity(traintestFile)

 prevPercent=fixedMinP
 for itr in range(maxIterations):
  randPercent=random.uniform(fixedMinP,fixedMaxP)
  absPercentDiff = abs(randPercent-prevPercent)
  if absPercentDiff<minDelta:
   if randPercent >= minDelta:
    randPercent = min(randPercent+minDelta, fixedMaxP)
   elif randPercent < prevPercent:
    randPercent = max(randPercent-minDelta, fixedMinP)
...
GenerateTrainTest2List(listtuple_pred_true_text, fixedMinP, fixedMaxP, minDelta)

...
maxTrainRatio =0.80
minTrainRatio = 0.60
trainFile = "/home/owner/PhD/dr.norbert/dataset/shorttext/tweet/2472/semisupervised/tweet_ensembele_train"
testFile = "/home/owner/PhD/dr.norbert/dataset/shorttext/tweet/2472/semisupervised/tweet_ensembele_test"
traintestFile = "/home/owner/PhD/dr.norbert/dataset/shorttext/tweet/2472/semisupervised/tweet_ensembele_traintest"
textsperlabelDir="/home/owner/PhD/dr.norbert/dataset/shorttext/tweet/2472/semisupervised/textsperlabel/"
dataFileTxtTrue = "/home/owner/PhD/dr.norbert/dataset/shorttext/tweet/2472/tweet-2472"
extClustFile = "/home/owner/PhD/dr.norbert/dataset/shorttext/tweet/2472/tweet-glove-dense-distmatrix-labels" 
...
def GenerateTrainTest2(number_text_train):
 trainDataRatio = 1.0
 ...
 for label, pred_true_txt in dic_tupple_class.items():
  ...
  #print("pred_true_txts="+str(len(pred_true_txts))+", pred_true_txts_good="+str(len(pred_true_txts_good)))


 for label, pred_true_txt in dic_tupple_class.items():
  pred_true_txts = dic_tupple_class[str(label)] 
  pred_true_txt_subs= []
  if len(pred_true_txts) > number_text_train:
   pred_true_txt_subs.extend(pred_true_txts[0:number_text_train])
   test_pred_true_txts.extend(pred_true_txts[number_text_train:len(pred_true_txts)]) 
  else:
 ...
 if trainDataRatio>=minTrainRatio and trainDataRatio<=maxTrainRatio:
...
def GenerateTrainTest2List(listtuple_pred_true_text):
 ...
 N_K = N/numberOfClusters			
 texts_Within_Training_Range = int(N_K*1.0 - N_K*0.7)
 Texts_Each_Block = texts_Within_Training_Range/5
 #if (N_K*0.7)%50==0 and N_K%50==0 and texts_Within_Training_Range>=200:
 # Texts_Each_Block = 50
 Texts_Each_Block=int(Texts_Each_Block)
 print("Texts_Each_Block="+str(Texts_Each_Block))
 start=int(N_K*0.7)
 end=int(N_K*1.0)+Texts_Each_Block
 for itr in range(iterations):
  for text_train in range(start,end,Texts_Each_Block):
   trainDataRatio = GenerateTrainTest2(text_train);
   if trainDataRatio<minTrainRatio or trainDataRatio>maxTrainRatio:
    continue
   print(str(itr)+","+str(text_train))
